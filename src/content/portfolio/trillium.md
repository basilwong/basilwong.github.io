...
Trillium is able to detect and correct single event upsets by using redundant computation and memory. Essentially, three of the same MCUs are running the same process at the same time. The changes in bits or state are detected when they compare memory and current process state. The design implements the comparison by comparing the values/states of two of the MCU's, then if they are different, determining that the third MCU is correct. The first and second MCU's then take on the state and values of the third MCU.
...
On the Communications and Data Handling team at UBC Orbit, I lead the development of Trillium:
